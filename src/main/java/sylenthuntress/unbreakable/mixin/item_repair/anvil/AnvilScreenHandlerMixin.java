...
public class AnvilScreenHandlerMixin {
    ...
    private static int noRepairCostScaling(int original) {
        if (!Unbreakable.CONFIG.anvilRepair.repairCost.ANVIL_SCALING()) original = 0;
        return original;
    }
    ...
    private Object scaleWithEnchantments(Object original, @Local(ordinal = 0) ItemStack inputStack) {
        ...
        if (Unbreakable.CONFIG.anvilRepair.repairCost.ENCHANTMENT_SCALING())
    ...
    private int scaleWithShatterLevel(int original, @Local(ordinal = 0) ItemStack inputStack, @Local(ordinal = 2) ItemStack secondInputStack, @Local(ordinal = 1) ItemStack outputStack) {
        ...
        if (scaledWithShatterLevel != newShatterLevel && Unbreakable.CONFIG.anvilRepair.repairCost.SHATTER_SCALING() && oldShatterLevel > newShatterLevel) {
    ...
    private int repairCostFactor(int original) {
        return Math.round(original * Unbreakable.CONFIG.anvilRepair.repairCost.MULTIPLIER());
    }
    ...
    private boolean disableTooExpensiveClient(boolean original) {
        return !Unbreakable.CONFIG.anvilRepair.tooExpensiveWarning();
    }
}
